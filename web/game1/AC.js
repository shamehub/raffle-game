...
$(function() {
	...
	$('.egg').on('click', function() {
		...
		$('.chuizi').addClass('jump').css({
			left: this.offsetLeft + 30,
			top: this.offsetTop
		});
		var that = $(this);
		$.get('../public/lucky-prize/prizes.json', function(data) {
			...
			setTimeout(function() {
				...
				$('.chuizi').removeClass('jump').css({top: '-.15rem', left: '2.65rem'});
				...
				_czc.push(["_trackEvent","游戏1","展现",data[num].name]);
				_czc.push(["_trackEvent","总计","展现",data[num].name]);
			}, 1000)
			
			$('.coupon-wrapper').on('click', function() {
				_czc.push(["_trackEvent","游戏1","点击",data[num].name]);
